...
RC FieldMeta::init(const char *name, AttrType attr_type, int attr_offset, int attr_len, bool visible, int field_id, bool nullable)
{
  ...
  if (AttrType::UNDEFINED == attr_type || attr_offset < 0 || attr_len <= 1) { // attr_len == 1 means only the "null indicator" byte exists
...
bool FieldMeta::is_field_null(const char *data) const { return static_cast<bool>(data[null_indicator_index()]); }

void FieldMeta::set_field_null_indicator(char *data, bool is_null) const { data[null_indicator_index()] = static_cast<char>(is_null); }

ssize_t FieldMeta::null_indicator_index() const { return offset() + len() - 1; }
...
RC FieldMeta::from_json(const Json::Value &json_value, FieldMeta &field)
{
  ...
  if(!nullable_value.isBool()) {
    LOG_ERROR("Nullable is not an bool value. json value=%s", nullable_value.toStyledString().c_str());
    return RC::INTERNAL;
  }
